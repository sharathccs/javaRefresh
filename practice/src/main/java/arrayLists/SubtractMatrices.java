...
static ArrayList<ArrayList<Integer>> subtractMatrices( ArrayList<ArrayList<Integer>> A, ArrayList<ArrayList<Integer>> B   ){
	
	ArrayList<ArrayList<Integer>> Difference = new ArrayList<ArrayList<Integer>>();
	int rowA = A.size();
	...
	for(int i=0, m=0; i<rowA && m<rowB ; i++, m++) {
		for(int j=0, n=0; j<colA && n<colB ; j++, n++) {
			diff = A.get(i).get(j)-B.get(m).get(n);
			Difference.get(i).add(j, diff);
		}
	}
